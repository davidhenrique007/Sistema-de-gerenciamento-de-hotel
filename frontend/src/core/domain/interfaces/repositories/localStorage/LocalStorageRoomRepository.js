...
import { createLogger } from '../../../utils.js'; // Ajuste o caminho conforme necessário
  ...
  _deserializeRoom(data) {
    try {
      ...
      return new Room({
        ...
        pricePerNight: new Money(data.pricePerNight.amount, data.pricePerNight.currency),
        status,
  ...
  async bulkUpdateStatus(roomIds, status) {
    ...
    try {
      ...
      for (const roomId of roomIds) {
        ...
        // Aqui seria necessário recriar o quarto com o novo status
        // Como as entidades são imutáveis, precisamos criar uma nova instância
        const currentRoom = allRooms[index];
        
        // Criar novo quarto com status atualizado (isso é uma simplificação)
        // Idealmente, a entidade Room teria um método para isso
      ...
      // Se houver IDs não encontrados, logar warning
