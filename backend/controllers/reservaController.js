...
// Função auxiliar para formatar moeda (uso interno)
...
class ReservaController {
  ...
  // =====================================================
  // LISTAR RESERVAS DO HÓSPEDE (Dashboard - usa guest_id do token JWT)
  // =====================================================
  async listarReservasCliente(req, res) {
    try {
      // Usar guest_id do token (UUID) - hóspede autenticado
      const guestId = req.user?.guest_id;
      
      if (!guestId) {
        console.log('⚠️ guest_id não encontrado no token');
        return res.status(401).json({ error: true, message: 'Cliente não identificado' });
      }
      
      console.log(`🔍 Buscando reservas para o guest: ${guestId}`);
  ...
  // =====================================================
  // LISTAR RESERVAS POR CLIENTE ID (Admin - usa clienteId numérico da URL)
  // =====================================================
  async listarReservasPorCliente(req, res) {
    try {
      const { clienteId } = req.params;

      console.log(`🔍 Buscando reservas para o cliente ID: ${clienteId}`);
